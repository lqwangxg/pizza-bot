...
module.exports = class StartConversationFlow extends Flow {
    ...
    async run(){
        ...
        const jsonIntent = JSON.stringify(this.context.intent);
        debug(`this.context.intent: ${jsonIntent}`);
        ...
        debug(`this.context.intent.name: ${this.context.intent.name}`);
        // Instantiate skill.
        ...
        debug(`check skill_status:${JSON.stringify(this.context.skill)}...`);
        // Log skill status.
        await this.slib.logger.skill_status(
          this.bot.extract_channel_id(), 
          this.bot.extract_sender_id(), 
          this.context.chat_id, 
          this.context.skill.type, "launched", {
            context: this.context
        });

        debug(`check context.skill.type:${this.context.skill.type}...`);
        // Add user's message to history header
        ...
        // Log chat.
        await this.slib.logger.chat(
          this.bot.extract_channel_id(), 
          this.bot.extract_sender_id(), 
          this.context.chat_id, 
          this.context.skill.type, 
          "user", 
          this.bot.extract_message());
