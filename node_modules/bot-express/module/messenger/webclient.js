...
module.exports = class MessengerWebClient {
  ...
  validate_signature(req){
    ...
    debug(`org_id signature:${signature}, org_id:${raw_ord_id}, raw_body:${raw_body}`);
    // Signature Validation
    let secret_key = "sha1=" + crypto.createHmac("sha1", this._org_id).update(this._org_id,"utf8","hex").digest("hex");
    debug(`org_id signature:${signature},  org_id:${raw_ord_id}, new secret hash org_id:${secret_key}`);
    if (secure_compare(secret_key, signature)){
      debug(`s: secret string is correct "${secret_key}","${signature}" `);
      return true
    }
    debug(`TODO: change org_id signature:${signature} ==== hash:${secret_key}`);
    return false
  }
