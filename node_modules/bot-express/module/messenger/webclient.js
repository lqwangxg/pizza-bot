...
module.exports = class MessengerWebClient {
  ...
  constructor(options){
      ...
      debug(`MessengerWebClient. response:${options.res}`);
  }
  ...
  async reply(event, messages){
    ...
    const jsonBody = { 
      "fulfillmentText": messages[0].text,
      "payload": {
        "google": {
    ...
    this._context.responseBody = jsonBody;
    debug(`jsonBody:${JSON.stringify(jsonBody)}`); 
    return jsonBody;
  }
  ...
  static extract_events(body){
    /*
    let event = body;
    event.message = body.queryResult;
    event.message.text = body.queryResult.queryText;
    event.parameters = body.queryResult.parameters;
    event.context = body.queryResult.outputContexts;
    event.intent = body.queryResult.intent;
    event.languageCode = body.queryResult.languageCode;
    event.sender_language = body.queryResult.languageCode;
    event.payload = body.originalDetectIntentRequest.payload;
    event.session = body.session?body.session : uuid;
    return [event];
    */
   return [body];
  }
  ...
  static extract_session_id(event){
    if(event.sessions){
      const matches = event.sessions.match(/[\w\-]+$/g);
      return matches?matches.toString(): uuid;
    } else {
      return uuid;
    }
  }
  ...
  static extract_param_value(event){
    return event.queryResult.parameters;
  }
  ...
  static extract_message_text(event){
    return event.queryResult.queryText;
  }
  ...
  static extract_postback_payload(event){
    const payload = {
      type: "intent",
      intent: { 
        name: event.queryResult.action,
        parameters: event.queryResult.parameters
      },
      language: event.queryResult.languageCode 
    };
    return JSON.stringify(payload);
  }
  static identify_message_type(message){
      let message_type;
      debug(`identify_message_type message:${message}`);
      if (message.text){
          // Type is text.
          message_type = "text";
  ...
  static compile_message(message_format, message_type, message){
    debug(`compile_message, message_format:${message_format}, message_type:${message_type}, message:${message} `);
