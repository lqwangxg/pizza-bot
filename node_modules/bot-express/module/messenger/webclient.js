...
module.exports = class MessengerWebClient {
  ...
  validate_signature(req){
    ...
    let hash = "sha1=" + crypto.createHmac("sha1", this._org_id).update(raw_body).digest("hex");
    debug(`org_id signature:${signature},  org_id:${raw_ord_id}, new secret hash org_id:${hash}`);
    if (secure_compare(hash, signature)){
      debug(`TODO: secret string is correct "${hash}","${signature}" `);
      return true
    }
    debug(`TODO: change org_id signature:${signature} ==== hash:${hash}`);
    return false
  }
